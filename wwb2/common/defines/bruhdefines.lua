...
NGame = {
	...
	GAME_SPEED_SECONDS = { 2.0, 0.5, 0.2, 0.08, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
...
NDiplomacy = {
	...
	TENSION_PEACE_FACTOR = 0.1,					-- scale of the amount of tension (from war declaration) reduced when peace is completed.
	TENSION_LIBERATE = -1,							-- Amount of tension generated by liberating a country. 
	TENSION_CAPITULATE = 1.5,						-- Scale of the amount of tension created by a countries capitulation.
	...
	JOINING_NAP_WAR_PENALTY = 0.3,					-- War support penalty for breaking non-breakable NAP
...
NCountry = {
	...
	VP_TO_SUPPLY_BASE = 3.5,							-- Bonus to supply from a VP, no matter the level
	...
	BASE_STABILITY_PARTY_POPULARITY_FACTOR = 0.20,	-- Default stability rulling party popularity factor
	...
	MAX_PROPAGANDA_STABILITY_IMPACT = -0.001,			-- Max total penalty from operative performing the propaganda mission in a country
	MAX_PROPAGANDA_WAR_SUPPORT_IMPACT = -0.001,		-- Max total penalty from operative performing the propaganda mission in a country
	...
	AIR_SUPPLY_CONVERSION_SCALE = 0.20,				-- Conversion scale for planes to air supply
	...
	GIE_HOST_CIC_FROM_LEGITIMACY_MAX = 2, 					--Host will receive from 0 to this value in CIC.
	GIE_HOST_MIC_FROM_LEGITIMACY_MAX = 2,					--Host will receive from 0 to this value in MIC.
	...
	GIE_EXILE_ARMY_LEADER_START_LEVEL = 4,	--Starting level for exile leader
	...
	SURRENDER_LIMIT_REDUCTION_PER_COLLABORATION = 0.2, 	--each percent of collaboration will lower surrender limit by this percentage 
...
NResistance = {
	...
	COMPLIANCE_GROWTH_BASE = 0.10, -- base compliance grow
	...
	COMPLIANCE_DECAY_AT_MAX_COMPLIANCE = -0.125, -- as compliance increases, it gets a decay rate depending on its value. compliance should stabilize at some value until its growth changes
	...
	SUPPRESSION_NEEDED_BY_RESISTANCE_POINT = 0.80, -- Number of suppression point we need for each 1% of resistance
...
NProduction = {
	...
	EFFICIENCY_LOSS_PER_UNUSED_DAY = 0.5,		-- Daily loss of efficiency for unused factory slots ( efficiency is tracked per factory slot in the production line )
	...
	ANNEX_FUEL_RATIO = 0.50,	-- How much fuel will be transferred on annexation
	...
	LICENSE_EQUIPMENT_BASE_SPEED = -0.20,				-- base MIC speed modifier for licensed equipment
	...
	LICENSE_EQUIPMENT_TECH_SPEED_MAX_YEARS = 3,			-- Maximum years for MIC speed modifier
	LICENSE_EQUIPMENT_SPEED_NOT_FACTION = -0.20,		-- MIC speed modifier for licensed equipment for not being in faction
	...
	MIN_NAVAL_EQUIPMENT_CONVERSION_RESOURCE_COST_FACTOR = 0.05,	-- Minimum fraction of an equipment type's base strategic resource cost to use when converting a naval equipment, such as through ship refitting.
...
NTechnology = {
	...
	BASE_YEAR_AHEAD_PENALTY_FACTOR = 3.4,		-- Base year ahead penalty
...
NBuildings = {
	...
	RADAR_RANGE_MIN = 50,				-- Radar range (from state center to province center) in measure of map pixels. Exluding techs.
...
NMilitary = {
	...
	INFRASTRUCTURE_MOVEMENT_SPEED_IMPACT = -0.01,	-- speed penalty per infrastucture below maximum.
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.045,        -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	...
	LAND_COMBAT_COLLATERAL_FACTOR = 0.0025,		   -- Factor to scale collateral damage to infra and forts with.
	...
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.07,		   -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	ATTRITION_EQUIPMENT_PER_TYPE_LOSS_CHANCE = 0.07, -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	COMBAT_MOVEMENT_SPEED = 0.30,	               -- speed reduction base modifier in combat
	...
	LAND_SPEED_MODIFIER = 0.04,                    -- basic speed control
	...
	BASE_LEADER_TRAIT_GAIN_XP = 0.65,			   -- Base xp gain for traits per hour for armies
	...
	ENEMY_AIR_SUPERIORITY_SPEED_IMPACT = -0.10,     -- effect on speed due to enemy air superiority
	...
	TRAINING_EXPERIENCE_SCALE = 80.0,
	TRAINING_ORG = 0.2,
	...
	TRAINING_ATTRITION = 0.02,		  			   -- amount of extra attrition from being in training
	...
	OVERSEAS_LOSE_EQUIPMENT_FACTOR = 0.8,		   -- percentage of equipment lost disbanded overseas
	...
	PLAYER_ORDER_PLANNING_DECAY = 0.1,				-- Amount of planning lost due to player manual order
	PLANNING_GAIN = 0.02,
	PLANNING_MAX = 0.2,                           	-- can get more from techs
	CIVILWAR_ORGANIZATION_FACTOR = 0.4,			  	-- Multiplier of org for both sides when civilwar.	
	...
	PLAN_MIN_AUTOMATED_EMPTY_POCKET_SIZE = 5,		-- The battle plan system will only automatically attack provinces in pockets that has no resistance and are no bigger than these many provinces
	...
	NUKE_MAX_DAMAGE_PERCENT = 0.7,					-- Minimum damage from nukes as a percentage of current strength/organisation
	...
	WITHDRAWING_SPEED_FACTOR = 0.20,				-- speed bonus when withdrawing
	...
	BASE_CAPTURE_EQUIPMENT_RATIO = 0.025,				-- after a successful land combat, ratio of the equipments that are being captured/salvaged from enemy's lost equipment
	...
	BORDER_WAR_WIN_DAYS_AGAINST_EMPTY_OPPONENTS = 21,		-- border wars will be automatically won if no opponent shows up for this duration
	
	
	MAX_RELATIVE_COMBAT_DAMAGE_TO_MODIFY_XP = 1.0,			-- you gain more XP if you are doing more damage relative to enemy, this is the max relative amount to gain following RATe
	...
	MIN_XP_RATE_TO_DECAY = 0.9,								-- minimum XP factor for dragged combats
	
	XP_GAIN_PER_OVERRUN_UNIT = 45.0,						-- fixed XP gain per overrun unit
	XP_GAIN_FOR_SHATTERING = 45.0,                          -- fixed XP gain per shattered unit
	...
	SURPLUS_SUPPLY_RATIO_FOR_ZERO_FUEL_FLOW = 1.0,		-- if a supply chunk has more supply needed than this ratio + 1 compared to its max supply flow, the units inside the chiunk will get no fuel 
...
NAir = {
	...
	AIR_WING_MAX_STATS_DEFENCE = 120,
	...
	AIR_NAVAL_KAMIKAZE_LOSSES_MULT = 5.0,          			-- Balancing value to increase usual losses if Kamikaze participating in the battle
	...
	ANTI_AIR_MAXIMUM_DAMAGE_REDUCTION_FACTOR = 0.70,	-- Maximum damage reduction factor applied to incoming air attacks against units with AA.
	...
	NAVAL_COMBAT_EXTERNAL_PLANES_MIN_CAP = 100,			-- Min cap for planes that can join naval combat
	...
	EFFICIENCY_REGION_CHANGE_DAILY_GAIN_STRATEGIC_BOMBER = 0.112,	-- How much efficiency to regain per day. Gain applied hourly.
	...
	DISRUPTION_FACTOR_CARRIER = 10.0,							-- multiplier on disruption damage to scale its effects on carrier vs carrier planes
	...
	BOMBING_TARGETING_RANDOM_FACTOR = 0.30,							-- % of picking the wrong target
	...
	BOMBING_INFRA_PRIO_SCALE = 0.5,									-- Scale of the selected priority for infastryctyre
...
NNavy = {
	...
	BEST_CAPITALS_TO_SCREENS_RATIO = 0.50, 							-- capitals / screens ratio used for creating FEX groups in naval combat
	...
	COMBAT_MAX_GROUPS = 2,										-- Max amount of "Fire Exchange" groups (FEX).
	...
	CONVOY_EFFICIENCY_LOSS_MODIFIER = 1.00,							-- How much efficiency drops when losing convoys. If modifier is 0.5, then losing 100% of convoys in short period, the efficiency will drop by 50%.
	...
	ALL_SHIPS_ACTIVATE_TIME = 24,                                    -- hours where all get to attack
	...
	CONVOY_SPOTTING_COOLDOWN_MIN = 48, -- minimum cooldown time
	...
	SUPREMACY_PER_SHIP_PER_MANPOWER = 0.08,							-- supremacy of a ship is calculated using its IC, manpower and a base define
	SUPREMACY_PER_SHIP_PER_IC = 0.008,
	SUPREMACY_PER_SHIP_BASE = 20.0,
	...
	NAVAL_MINES_PLANTING_SPEED_MULT = 0.001,						-- Value used to overall balance of the speed of planting naval mines
	...
	BASE_ESCAPE_SPEED = 0.060,										-- daily base escape speed (gained as percentagE)
	SPEED_TO_ESCAPE_SPEED = 1.15,									-- ratio to converstion from ship speed to escape speed (divided by hundred)
	ESCAPE_SPEED_PER_COMBAT_DAY = 0.01,								-- daily increase in escape speed during combat duration
	MAX_ESCAPE_SPEED_FROM_COMBAT_DURATION = 0.20,					-- max escape speed that will be gained from combat duration
	ESCAPE_SPEED_SUB_BASE = 0.20,									-- subs get faster escape speed. gets replaced by hidden version below if hidden
	ESCAPE_SPEED_HIDDEN_SUB = 0.30,									-- hidden subs get faster escape speed
	...
	SUB_DETECTION_CHANCE_SPOTTING_SPEED_EFFECT = 1.0,				-- effect of spotting speed for initial spotting of pure submarine forces. this along with prev value is added together and rolled against a random to start spotting
	...
	NAVAL_INVASION_SPOTTING_SPEED_MULT = 1.0,						-- spotting speed mult against naval invasion armies
	...
	UNIT_TRANSFER_DETECTION_CHANCE_BASE = 4.12,							-- unit transfer and naval invasion base chance detection percentage (if this fails, no detection is done on that tick)
	...
	LIGHT_GUN_ATTACK_TO_SHORE_BOMBARDMENT							= 0.01, -- light gun attack value is divided by this value * 100 and added to shore bombardment modifier 
	...
	DEPTH_CHARGES_HIT_CHANCE_MULT 									= 1.25, 		-- multiplies hit chance of small guns
	...
	CONVOY_DEFENSE_MAX_CONVOY_TO_SHIP_RATIO							= 30.0,		-- each ship in convoy defense mission can at most cover this many convoys without losing efficiency
	...
	SUBMARINE_HIDE_TIMEOUT 											= 30,		-- Amount of in-game-hours that takes the submarine (with position unrevealed), to hide.
	SUBMARINE_REVEALED_TIMEOUT 										= 24,		-- Amount of in-game-hours that makes the submarine visible if it is on the defender side.
	...
	SUBMARINE_BASE_TORPEDO_REVEAL_CHANCE 							= 0.045,		-- Chance of a submarine being revealed when it fires. 1.0 is 100%. this chance is then multiplied with modifier created by comparing firer's visibiility and target's detection
...
NAI = {
	...
	SCREENS_TO_CAPITAL_RATIO = 2.0,					-- screens to capital/carrier count in carrier & capital taskforces
...
NFocus = {
	...
	MAX_SAVED_FOCUS_PROGRESS = 21,				-- This much progress can be saved while not having a focus selected
},
